@extends('admin.layouts.master')
@section('title')
    اقسام المنتجات
@endsection
@section('content')
    <h4 class="py-3">
        <a href="{{ route('admin.dashboard') }}">
            <span class="text-muted fw-light">الرئيسية/
            </span>
        </a>
        اقسام المنتجات
    </h4>
    <a class="btn btn-primary mb-2" href="{{ route('admin.categories.create') }}">اضافة قسم جديد</a>

    <div class="card">
        <div class="table-responsive p-2">
            <table class="myDatatable table table-bordered">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>اسم القسم</th>
                        <th>الصورة</th>
                        <th>الحالة</th>
                        <th>الخيارات</th>

                    </tr>
                </thead>
                <tbody>
                    @foreach ($categories as $cat)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $cat->name }}</td>
                            <td>
                                @if ($cat->image)
                                    <img src="{{ asset('Files/images/categories/' . $cat->image) }}"
                                        style="width: 50px; height:50px;">
                                @else
                                    ----
                                @endif
                            </td>
                            <td>{{ $cat->status() }}</td>
                            <td>
                                <div class="m-2">

                                    <a href="{{ route('admin.categories.edit', $cat->id) }}" class="btn btn-info btn-sm"
                                        role="button" aria-pressed="true" title="تعديل">
                                        <i class="fa fa-edit"></i></a>


                                    <a onclick="fireDeleteEvent({{ $cat->id }})" type="button" title="حذف"
                                        data-toggle="tooltip" class="btn btn-danger btn-sm text-white"><i
                                            class="fas fa-trash-alt"></i></a>

                                    <form action="{{ route('admin.categories.destroy', $cat->id) }}" method="POST"
                                        id="form-{{ $cat->id }}">
                                        @csrf
                                        @method('Delete')
                                    </form>
                                </div>
                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
@endsection
